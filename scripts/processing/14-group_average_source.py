...
from scripts.processing.library.config import (meg_dir, subjects_dir, N_JOBS, smooth,
                            fsaverage_vertices, l_freq, exclude_subjects)

exclude_subjects = [1, 3, 4, 5, 7, 8, 9, 10, 11, 12, 13, 15, 16, 17]

# compute_morph_matrix CHANGE FOR MNE 0.21.0 VERSION TO mne.compute_source_morph()
# morph_precomputed to mne.compute_source_morph()
...
def morph_stc(subject_id):
    ...
    for condition in ('contrast', 'faces_eq', 'scrambled_eq'):
        ...
        morphed = stc.compute_source_morph('fsaverage', fsaverage_vertices,
                                       morph_mat)
